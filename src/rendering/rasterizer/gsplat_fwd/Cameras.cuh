...
inline __device__ glm::fvec3 apply_row_major_transform_vector(
    const float* const transform_row_major_4x4,
    glm::fvec3 const& vector) {
    return glm::fvec3{
        transform_row_major_4x4[0] * vector.x + transform_row_major_4x4[1] * vector.y +
            transform_row_major_4x4[2] * vector.z,
        transform_row_major_4x4[4] * vector.x + transform_row_major_4x4[5] * vector.y +
            transform_row_major_4x4[6] * vector.z,
        transform_row_major_4x4[8] * vector.x + transform_row_major_4x4[9] * vector.y +
            transform_row_major_4x4[10] * vector.z,
    };
}
...
world_gaussian_to_image_gaussian_linearized_shutter_pose(
    ...
    const auto projected_world_mean =
        model_transform_row_major_4x4 != nullptr
            ? apply_row_major_transform_point(
                  model_transform_row_major_4x4, gaussian_world_mean)
            : gaussian_world_mean;
    auto const [image_mean, mean_valid] =
        camera_model.template world_point_to_image_point_shutter_pose<>(
            projected_world_mean,
            rolling_shutter_parameters,
            margin_factor);
    if (!mean_valid) {
    ...
    auto const gaussian_camera_mean =
        glm::rotate(shutter_pose.q, projected_world_mean) + shutter_pose.t;
...
#pragma unroll
    for (auto i = 0u; i < 3; ++i) {
        auto world_axis = gaussian_world_scale[i] * gaussian_rotation[i];
        if (model_transform_row_major_4x4 != nullptr) {
            world_axis = apply_row_major_transform_vector(
                model_transform_row_major_4x4, world_axis);
        }
